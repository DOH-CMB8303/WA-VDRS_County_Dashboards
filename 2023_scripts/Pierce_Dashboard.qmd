---
title: "Pierce County"
subtitle: "National Violent Death Reporting System (2015-2023)"
format: 
  dashboard:
...
nvdrs_var_pierce<-readRDS("Y:/Confidential/HSQA/CHS/Washington Violent Death Reporting System/County Level VDRS Dashboards/VDRS County Dashboards/2023_Dashboards/nvdrs_var_pierce.rds")

# creating subsets based on manner of death/weapon type
# Homicide
nvdrs_homicide <- nvdrs_var_pierce %>% 
         filter(AbstractorDeathmanner_c %in% c("Homicide"))

# Suicide
nvdrs_suicide <- nvdrs_var_pierce %>% 
         filter(AbstractorDeathmanner_c %in% c("Suicide or intentional self-harm"))

# Unintentional firearm
nvdrs_firearm <- nvdrs_var_pierce %>% 
         filter(WeaponType1 %in% c("Firearm"))
```

# Homicide

## Row 1- Number of incidents and demographics table

### Column 1 - Number of incidents over time
...
### Column 2 - Mental Health/substance abuse, Relationship/life stressors, Crime (Mental Health/substance abuse)
...
# Define the variables you're working with
...
# Convert to long format and complete missing (zero count) variables
...
# converting age numeric into age groups
nvdrs_suicide <- nvdrs_suicide %>%
  mutate(AgeGroup = case_when(
    AgeYears_c <= 17 ~ "0-17",
    AgeYears_c > 17 & AgeYears_c <= 25 ~ "18-25",
...
mech_suicide_long <- mech_suicide %>%
  ...
  mutate(Value = ifelse(is.na(Value), "Unknown", as.character(Value))) %>%
  group_by(Variable, Value) %>%
  summarise(Count = n(), .groups = "drop") %>%
  mutate(Percent = round(Count / sum(Count) * 100, 1)) %>%
...
count_firearm <- count_firearm %>%
  mutate(IncidentYear = factor(IncidentYear, levels = c("2015","2016","2017","2018", "2019", "2020", "2021", "2022","2023")))
...
firearm_time <- ggplot(count_firearm, aes(x = IncidentYear, y = count, group = WeaponType1)) +
  ...
  scale_x_discrete(limits = c("2015","2016","2017","2018", "2019", "2020", "2021", "2022","2023")) +  # Ensure correct order
...
#table
firearm_demo_tbl <- nvdrs_homicide %>% 
  select(
    AgeGroup, 
    Sex, 
    RaceEthnicity_c, 
